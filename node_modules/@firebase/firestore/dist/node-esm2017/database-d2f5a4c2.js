...
class GrpcConnection {
        ...
        return nodePromise((callback) => {
            ...
            return stub[rpcName](jsonRequest, metadata, (grpcError, value) => {
                if (grpcError) {
                    ...
                    logDebug(LOG_TAG, `RPC '${rpcName}' completed with response:`, value);
    ...
    invokeStreamingRPC(rpcName, path, request, token) {
        ...
        logDebug(LOG_TAG, `RPC '${rpcName}' invoked (streaming) with request:`, request);
        const stub = this.ensureActiveStub();
        const metadata = createMetadata(this.databasePath, token);
    ...
    openStream(rpcName, token) {
        ...
        grpcStream.on('error', (grpcError) => {
            if (!closed) {
                logWarn(LOG_TAG, 'GRPC stream error. Code:', grpcError.code, 'Message:', grpcError.message);
                const code = mapCodeFromRpcCode(grpcError.code);
...
function applyMutationToLocalView(mutation, maybeDoc, baseDoc, localWriteTime) {
    if (mutation instanceof SetMutation) {
        return applySetMutationToLocalView(mutation, maybeDoc, localWriteTime, baseDoc);
    }
    else if (mutation instanceof PatchMutation) {
        return applyPatchMutationToLocalView(mutation, maybeDoc, localWriteTime, baseDoc);
    }
    else {
...
function applySetMutationToLocalView(mutation, maybeDoc, localWriteTime, baseDoc) {
...
function applyPatchMutationToLocalView(mutation, maybeDoc, localWriteTime, baseDoc) {
    if (!preconditionIsValidForDocument(mutation.precondition, maybeDoc)) {
        return maybeDoc;
    }
    const version = getPostMutationVersion(maybeDoc);
 ...
 * limitations under the License.
 */
const LOG_TAG$4 = 'ComponentProvider';
/**
...
function removeComponents(firestore) {
    ...
    if (datastore) {
        logDebug(LOG_TAG$4, 'Removing Datastore');
...
class SimpleDbTransaction {
    ...
    abort(error) {
        ...
        if (!this.aborted) {
            logDebug(LOG_TAG$5, 'Aborting transaction:', error ? error.message : 'Client-initiated abort');
            this.aborted = true;
...
class SimpleDb {
    ...
    static delete(name) {
        logDebug(LOG_TAG$5, 'Removing database:', name);
    ...
    async ensureDb(action) {
        if (!this.db) {
            logDebug(LOG_TAG$5, 'Opening database:', this.name);
            this.db = await new Promise((resolve, reject) => {
                ...
                request.onupgradeneeded = (event) => {
                    logDebug(LOG_TAG$5, 'Database "' + this.name + '" requires upgrade from version:', event.oldVersion);
                    const db = event.target.result;
                    this.schemaConverter
                        .createOrUpgrade(db, request.transaction, event.oldVersion, this.version)
                        .next(() => {
                        logDebug(LOG_TAG$5, 'Database upgrade to version ' + this.version + ' complete');
                    });
                };
            });
        }
    ...
    async runTransaction(action, mode, objectStores, transactionFn) {
        ...
        while (true) {
            ...
            try {
                ...
                logDebug(LOG_TAG$5, 'Transaction failed with error:', error.message, 'Retrying:', retryable);
                this.close();
                if (!retryable) {
...
class SimpleDbStore {
    ...
    put(keyOrValue, value) {
        let request;
        if (value !== undefined) {
            logDebug(LOG_TAG$5, 'PUT', this.store.name, keyOrValue, value);
            request = this.store.put(value, keyOrValue);
        }
        else {
            logDebug(LOG_TAG$5, 'PUT', this.store.name, '<auto-key>', keyOrValue);
     ...
     */
    add(value) {
        logDebug(LOG_TAG$5, 'ADD', this.store.name, value, value);
    ...
    get(key) {
        ...
        return wrapRequest(request).next(result => {
            ...
            logDebug(LOG_TAG$5, 'GET', this.store.name, key, result);
            return result;
        });
    }
    delete(key) {
        logDebug(LOG_TAG$5, 'DELETE', this.store.name, key);
     ...
     */
    count() {
        logDebug(LOG_TAG$5, 'COUNT', this.store.name);
    ...
    deleteAll(indexOrRange, range) {
        logDebug(LOG_TAG$5, 'DELETE ALL', this.store.name);
 ...
 * limitations under the License.
 */
const LOG_TAG$6 = 'LruGarbageCollector';
        ...
        this.gcTask = this.asyncQueue.enqueueAfterDelay("lru_garbage_collection" /* LruGarbageCollection */, delay, async () => {
            ...
            try {
                ...
                if (isIndexedDbTransactionError(e)) {
                    logDebug(LOG_TAG$6, 'Ignoring IndexedDB error during garbage collection: ', e);
                }
                else {
...
function fromDbMutationBatch(localSerializer, dbBatch) {
    ...
    for (let i = dbBatch.mutations.length - 1; i >= 0; --i) {
        ...
        if ((mutationProto === null || mutationProto === void 0 ? void 0 : mutationProto.transform) !== undefined) {
 ...
 * limitations under the License.
 */
const LOG_TAG$7 = 'IndexedDbPersistence';
/**
...
class IndexedDbPersistence {
    ...
    forceOwningTab) {
        ...
        if (this.window && this.window.localStorage) {
            ...
            if (forceOwningTab === false) {
                logError(LOG_TAG$7, 'LocalStorage is unavailable. As a result, persistence may not work ' +
    ...
    updateClientMetadataAndTryBecomePrimary() {
        return this.runTransaction('updateClientMetadataAndTryBecomePrimary', 'readwrite', txn => {
            ...
            .catch(e => {
            if (isIndexedDbTransactionError(e)) {
                logDebug(LOG_TAG$7, 'Failed to extend owner lease: ', e);
            ...
            logDebug(LOG_TAG$7, 'Releasing owner lease after error during lease refresh', e);
    ...
    canActAsPrimary(txn) {
        ...
        return store
            ...
            .next(canActAsPrimary => {
            if (this.isPrimary !== canActAsPrimary) {
                logDebug(LOG_TAG$7, `Client ${canActAsPrimary ? 'is' : 'is not'} eligible for a primary lease.`);
            }
    ...
    runTransaction(action, mode, transactionOperation) {
        logDebug(LOG_TAG$7, 'Starting transaction:', action);
    ...
    releasePrimaryLeaseIfHeld(txn) {
        ...
        return store.get(DbPrimaryClient.key).next(primaryClient => {
            if (this.isLocalClient(primaryClient)) {
                logDebug(LOG_TAG$7, 'Releasing primary lease.');
    ...
    isClientZombied(clientId) {
        ...
        try {
            ...
            logDebug(LOG_TAG$7, `Client '${clientId}' ${isZombied ? 'is' : 'is not'} zombied in LocalStorage`);
            return isZombied;
        }
        catch (e) {
            // Gracefully handle if LocalStorage isn't working.
            logError(LOG_TAG$7, 'Failed to get zombied client id.', e);
            return false;
        }
    }
    /**
    ...
    return documentBuffer.getEntries(txn, updatedKeys).next(existingDocs => {
        ...
        documents.forEach((key, doc) => {
            ...
            if (doc instanceof NoDocument &&
                ...
                logDebug(LOG_TAG$8, 'Ignoring outdated watch update for ', key, '. Current version:', existingDoc.version, ' Watch version:', doc.version);
            }
        });
        return changedDocs;
    });
...
async function notifyLocalViewChanges(localStore, viewChanges) {
    ...
    try {
        ...
        if (isIndexedDbTransactionError(e)) {
            ...
            logDebug(LOG_TAG$8, 'Failed to update sequence numbers: ' + e);
        }
        else {
            throw e;
        }
...
async function releaseTarget(localStore, targetId, keepPersistedTargetData) {
    ...
    try {
        ...
        if (isIndexedDbTransactionError(e)) {
            ...
            logDebug(LOG_TAG$8, `Failed to update sequence numbers for target ${targetId}: ${e}`);
        }
        else {
            throw e;
        }
 ...
 * queue.
 */
async function applyBundleDocuments(localStore, bundleConverter, documents, bundleName) {
    const localStoreImpl = debugCast(localStore);
 ...
 * limitations under the License.
 */
const LOG_TAG$b = 'OnlineStateTracker';
        ...
        this.connectivityMonitor.addCallback((_) => {
            asyncQueue.enqueueAndForget(async () => {
                ...
                if (canUseNetwork(this)) {
                    logDebug(LOG_TAG$c, 'Restarting streams for network reachability change.');
...
async function remoteStoreShutdown(remoteStore) {
    const remoteStoreImpl = debugCast(remoteStore);
    logDebug(LOG_TAG$c, 'RemoteStore shutting down.');
...
async function onWatchStreamChange(remoteStoreImpl, watchChange, snapshotVersion) {
    ...
    if (watchChange instanceof WatchTargetChange &&
        ...
        try {
            ...
            logDebug(LOG_TAG$c, 'Failed to remove targets %s: %s ', watchChange.targetIds.join(','), e);
    ...
    if (!snapshotVersion.isEqual(SnapshotVersion.min())) {
        try {
            ...
            logDebug(LOG_TAG$c, 'Failed to raise snapshot:', e);
            await disableNetworkUntilRecovery(remoteStoreImpl, e);
        }
    }
}
...
async function disableNetworkUntilRecovery(remoteStoreImpl, e, op) {
    if (isIndexedDbTransactionError(e)) {
        ...
        remoteStoreImpl.asyncQueue.enqueueRetryable(async () => {
            logDebug(LOG_TAG$c, 'Retrying IndexedDB access');
            await op();
...
async function remoteStoreHandleCredentialChange(remoteStore, user) {
    ...
    logDebug(LOG_TAG$c, 'RemoteStore received new credentials');
    ...
    if (!remoteStoreImpl.writeStream) {
        ...
        remoteStoreImpl.onNetworkStatusChange.push(async (enabled) => {
            if (enabled) {
                ...
                if (remoteStoreImpl.writePipeline.length > 0) {
                    logDebug(LOG_TAG$c, `Stopping write stream with ${remoteStoreImpl.writePipeline.length} pending writes`);
...
class BundleLoader {
     ...
     */
    async complete() {
        const changedDocuments = await applyBundleDocuments(this.localStore, new BundleConverterImpl(this.serializer), this.documents, this.bundleMetadata.id);
...
async function registerPendingWritesCallback(syncEngine, callback) {
    ...
    if (!canUseNetwork(syncEngineImpl.remoteStore)) {
        logDebug(LOG_TAG$e, 'The network is disabled. The task returned by ' +
...
function updateTrackedLimbos(syncEngineImpl, targetId, limboChanges) {
    for (const limboChange of limboChanges) {
        if (limboChange instanceof AddedLimboDocument) {
            ...
            logDebug(LOG_TAG$e, 'Document no longer in limbo: ' + limboChange.key);
...
function trackLimboChange(syncEngineImpl, limboChange) {
    ...
    if (!syncEngineImpl.activeLimboTargetsByKey.get(key)) {
        logDebug(LOG_TAG$e, 'New document in limbo: ' + key);
...
async function syncEngineHandleCredentialChange(syncEngine, user) {
    ...
    if (userChanged) {
        logDebug(LOG_TAG$e, 'User change. New user:', user.toKey());
...
async function applyBatchState(syncEngine, batchId, batchState, error) {
    ...
    if (documents === null) {
        ...
        // as applicable.
        logDebug(LOG_TAG$e, 'Cannot apply mutation batch with id: ' + batchId);
        return;
    }
...
async function applyTargetState(syncEngine, targetId, state, error) {
    ...
    if (syncEngineImpl._isPrimaryClient) {
        ...
        logDebug(LOG_TAG$e, 'Ignoring unexpected query state notification.');
        return;
    }
    if (syncEngineImpl.queriesByTarget.has(targetId)) {
        switch (state) {
...
async function applyActiveTargetsChange(syncEngine, added, removed) {
    ...
    for (const targetId of added) {
        if (syncEngineImpl.queriesByTarget.has(targetId)) {
            ...
            logDebug(LOG_TAG$e, 'Adding an already active target ' + targetId);
            continue;
        }
        const target = await getCachedTarget(syncEngineImpl.localStore, targetId);
 ...
 * limitations under the License.
 */
const LOG_TAG$f = 'FirestoreClient';
...
class FirestoreClient {
    ...
    asyncQueue, databaseInfo) {
        ...
        this.credentials.setChangeListener(user => {
            logDebug(LOG_TAG$f, 'Received user=', user.uid);
...
async function setOfflineComponentProvider(client, offlineComponentProvider) {
    ...
    logDebug(LOG_TAG$f, 'Initializing OfflineComponentProvider');
...
async function setOnlineComponentProvider(client, onlineComponentProvider) {
    ...
    logDebug(LOG_TAG$f, 'Initializing OnlineComponentProvider');
    const configuration = await client.getConfiguration();
...
async function ensureOfflineComponents(client) {
    if (!client.offlineComponents) {
        logDebug(LOG_TAG$f, 'Using default OfflineComponentProvider');
...
async function ensureOnlineComponents(client) {
    if (!client.onlineComponents) {
        logDebug(LOG_TAG$f, 'Using default OnlineComponentProvider');
...
//# sourceMappingURL=database-d2f5a4c2.js.map
